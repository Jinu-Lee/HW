...
Node dummy = {0, 0, 0, {LONG_MAX, 999999999}, {&dummy, &dummy}}, *dummy_ptr = &dummy;
...
Node* find_min_count_node(Node* root) {
    ...
    Time left = find_min_count_node(root->child[0])->access_time;
    Time right = find_min_count_node(root->child[1])->access_time;
    Node* min = (compare(left, right)) ? root->child[0] : root->child[1];
    if (min == dummy_ptr) {
        return root;
    } else {
        return (compare(root->access_time, min->access_time)) ? root : min;
    }
}
